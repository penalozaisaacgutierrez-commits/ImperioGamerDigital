/**
 * ImperioGamerDigital — JavaScript principal
...
(function () {
  ...
  var CONSENT_KEY = "igd_cookie_consent";
  ...
  function siteOrigin() {
    return String(cfg.sitioUrl || "https://imperiogamerdigital.onrender.com/").replace(
      /\/?$/,
      "/"
    );
  }
  ...
  function injectSeoExtras() {
    ...
    var names = {
      ...
      "ubicacion.html": "Ubicación en Gaira",
    ...
    if (cfg.geoLat != null && cfg.geoLng != null) {
      injectJsonLd("ld-local", {
        ...
        address: {
          "@type": "PostalAddress",
          streetAddress: "Edf. Fuentemar, Cra. 1 No. 7-69, Gaira",
          addressLocality: cfg.ciudad || "Santa Marta",
          addressRegion: cfg.departamento || "Magdalena",
          postalCode: cfg.codigoPostal || "470006",
          addressCountry: "CO",
        },
        geo: {
          "@type": "GeoCoordinates",
        ...
        hasMap: cfg.mapsUrl,
        areaServed: ["Santa Marta", "Medellín", "Colombia"],
        priceRange: "$$",
      });
    }
  }
  ...
  var FALLBACK =
    cfg.imagenFallback ||
    "https://images.unsplash.com/photo-1542751371-adc38448a05e?w=1200&auto=format&fit=crop&q=75";
  ...
  function injectConfig() {
    ...
    $$("[data-map]").forEach(function (iframe) {
      var q = encodeURIComponent(
        cfg.mapaQuery || cfg.direccion || "Gaira, Santa Marta, Magdalena, Colombia"
      );
      iframe.src =
    ...
    $$("[data-cfg-href='mapsUrl']").forEach(function (el) {
      el.href =
        ...
        "https://www.google.com/maps/search/?api=1&query=" +
          encodeURIComponent(cfg.mapaQuery || cfg.direccion || "Gaira, Santa Marta");
    });
  }
  ...
  function initForm() {
    ...
    form.addEventListener("submit", function (e) {
      ...
      var composed =
        "Consulta desde el sitio web de ImperioGamerDigital\n" +
        "ID de campaña: " +
        (cfg.idCampana || "") +
        "\nNombre: " +
        nombre +
  ...
  function buildSerpAdCard(opts) {
    ...
    return (
      ...
      '<span class="gads-serp-favicon" aria-hidden="true">IG</span>' +
  ...
  function buildGoogleAdsSection() {
    ...
    var variants = [
      {
        query: "tienda gamer medellin",
        headline: gadsHeadlineText(titulares, [0, 1, 4]),
        description: descripciones[0] ? descripciones[0].texto : "",
      },
      {
        query: "computadores gamer",
        headline: gadsHeadlineText(titulares, [3, 2, 8]),
        description: descripciones[1] ? descripciones[1].texto : "",
      },
      {
        query: "tienda gamer Santa Marta",
        headline: gadsHeadlineText(titulares, [5, 6, 11]),
        description: descripciones[2] ? descripciones[2].texto : "",
      },
      {
        query: "setup gamer",
        headline: gadsHeadlineText(titulares, [7, 12, 13]),
    ...
    var html =
      ...
      '<p class="gads-preview-label">Así se verá en Google · resultados de búsqueda</p>' +
    ...
    html +=
      "</div>" +
      '<p class="gads-preview-note">Google combina hasta tres titulares y una descripción. Estas cuatro vistas cubren quiénes somos, a qué nos dedicamos, los más de 10 años del sitio y la sede en Gaira. URL final: <a href="' +
      sitioUrl +
      '" target="_blank" rel="noopener noreferrer">' +
      escapeHtml(sitioUrl) +
      "</a>. La etiqueta «Patrocinado» identifica el anuncio. La visibilidad orgánica en Search Console es un canal distinto: no se finge ni se oculta.</p></div>" +
    ...
    html +=
      ...
      "</span> Concordancia exacta <small>8 palabras aptas · copiar con corchetes [ ] en Google Ads</small></h3><p class=\"gads-exact-lead\">En verde: coinciden con el sitio (quiénes somos, tienda gamer Medellín, sede en Gaira). No prometen apuestas, software ilegal ni regalos engañosos.</p><div class=\"gads-keywords-cloud\">";
